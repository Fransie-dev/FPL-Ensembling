...
def select_team(expected_scores, prices, positions, clubs, total_budget=100, sub_factor=SUB_FACTOR):
    ...
    # Squad constraints
    model += sum(decisions) == 11  # squad size
    model += sum(captain_decisions) == 1  # 1 captain
    
    for i in range(num_players):  
        model += (decisions[i] - captain_decisions[i]) >= 0  # captain must be on team
...
class TransferOptimiser:
    ...
    def apply_transfer_constraints(self, GW, model, transfer_in_decisions_free,  transfer_in_decisions, transfer_out_decisions, budget_now):
        # only 1 free transfer
        # if GW in FREE_TRANSFER:
        #         model += sum(transfer_in_decisions_free) <= 2
        # else:
                # model += sum(transfer_in_decisions_free) <= 1 # Else
    ...
    def solve(self, current_squad_indices, budget_now, sub_factor, GW):
        ...
        # transfer_penalty = sum(transfer_in_decisions_paid) if GW in FREE_HIT  else sum(transfer_in_decisions_paid) * 4 # 4-pt incremental hit

        self.apply_transfer_constraints(model, GW, transfer_in_decisions_free, transfer_in_decisions, transfer_out_decisions, budget_now)
...
def manage_squad(df_repeated, GW, squad_indices, budget = 0):
    ...
    transfer_in_decisions, transfer_out_decisions, decisions, sub_decisions, captain_decisions = opt.solve(squad_indices, budget_now=budget, sub_factor=SUB_FACTOR, GW = GW)
    ...
    print(penalty)
    # penalty = 0 if GW in FREE_HIT  else ((len(p_in) - 1) * 4)
    exp_captain_point = expected_scores.iloc[captain_indice].sum()*2 if GW in TRIPLE_CAPTAIN else expected_scores.iloc[captain_indice].sum()
    act_captain_point = actual_scores.iloc[captain_indice].sum()*2 if GW in TRIPLE_CAPTAIN else actual_scores.iloc[captain_indice].sum()
    ...
    week_transfers = len(p_in) 
    next_week_free_transfer = [GW + 1] if week_transfers == 0 else [0]
    ...
    return squad_indices, starting_indices, sub_indices, captain_indice, budget_new, actual_points, week_transfers
...
use = 'LR' # Possible values: 'LR', 'CBoost', 'SVR', 'KNN', 'MLP', 'Meta'
total_points, squads, transfers = [], [], []
df_repeated = get_repeat(use) # Replicate all players`
SUB_FACTOR = 0.2
WILDCARD_1 = 12
WILDCARD_2 = 18
WILDCARD = [12, 18] # Cannot use in GW 1
TRIPLE_CAPTAIN = [1]
BENCH_BOOST = [11]
FREE_HIT = [5] # Cannot use in GW 1
BUDGET = 100
FREE_TRANSFER = [2] # One free transfer in second game
# %%
for GW in list(range(1,39)):
    ...
    # Main Loop
    if GW == 1:
        squad_indices, starting_indices, sub_indices, captain_indice, budget, actual_points =  get_starting_XV(df_repeated, max_budget = 100, GW = GW) # For the first gameweek
    else:
        squad_indices, starting_indices, sub_indices, captain_indice, budget, actual_points, week_transfers = manage_squad(df_repeated, GW, squad_indices, budget)
        transfers.append(week_transfers)
...
print(f'Total transfers: {sum(transfers)}')
df = pd.concat(squads) 
# %%
# 2245 sonder wildcards, 2394
# Can now only achieve = 2146 with wildcards
# df.to_csv('./old/team_preds.csv', index = False)
# print(df.head(15*4).to_latex())
# CBoost - 2005
# %%# %%
# Chip strategy
