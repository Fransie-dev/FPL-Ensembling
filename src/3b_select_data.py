...
def mi_mrmr(X_train, y_train, X_test, y_test, max_feat = 100, min_feat = 10, step_size = 5, algos = ['mrmr', 'mi']):
    ...
    for algo, label, color in zip(['mrmr', 'mi'], ['MRMR', 'Mutual Info'], ['orangered', 'blue']):
            plt.plot(loss.index, loss[algo], label = label, color = color, lw = 3)
