...
def drop_dups(df):
    ...
    identifiers = ['element', 'fixture', 'id', 'roster_id']
    teams = ['name', 'short_name', 'h_team', 'a_team']
    dups = ['h_goals', 'a_goals', 'goals', 'date', 'time']
    for col in identifiers + teams + dups:
            if col in df.columns:
                df.drop(columns = col, inplace = True)
    # print('No duplicate rows exist: ', df[df.duplicated()])
    # print(df[df.duplicated(['player_name', 'GW'])]['player_name'])
